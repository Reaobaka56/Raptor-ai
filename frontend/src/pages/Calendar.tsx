...
const TYPE_DOT: Record<string, string> = {
  review: 'bg-red-400', planning: 'bg-indigo-400', retro: 'bg-amber-400', sync: 'bg-green-400'
}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-xs text-gray-600 mb-1 block">Date</label>
            <input type="date" value={date} onChange={e => setDate(e.target.value)}
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30" /></div>
          <div><label className="text-xs text-gray-600 mb-1 block">Time</label>
            <input type="time" value={time} onChange={e => setTime(e.target.value)}
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-xs text-gray-600 mb-1 block">Duration</label>
          ...
          <div><label className="text-xs text-gray-600 mb-1 block">Type</label>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-[#0d0d14] p-5">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex gap-1">
                <button onClick={prevMonth} className="rounded border border-white/10 p-1.5 text-gray-500 hover:text-white transition"><ChevronLeft className="h-4 w-4" /></button>
                <button onClick={() => { setYear(now.getFullYear()); setMonth(now.getMonth()) }}
                  className="rounded border border-white/10 px-3 py-1 text-xs text-gray-500 hover:text-white transition">Today</button>
                <button onClick={nextMonth} className="rounded border border-white/10 p-1.5 text-gray-500 hover:text-white transition"><ChevronRight className="h-4 w-4" /></button>
              </div>
            </div>
            <div className="grid grid-cols-7 mb-2">
              {DAYS.map(d => <div key={d} className="text-center text-[10px] font-mono uppercase text-gray-600 py-1">{d}</div>)}
            </div>
            <div className="grid grid-cols-7 gap-px bg-white/5 rounded-lg overflow-hidden">
              {Array.from({ length: firstDay }).map((_, i) => <div key={`e${i}`} className="bg-[#0d0d14] aspect-square" />)}
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button key={day} onClick={() => setSelectedDay(isSelected ? null : day)}
                    className={`bg-[#0d0d14] aspect-square flex flex-col items-center justify-start pt-1.5 px-1 hover:bg-white/5 ${isSelected ? 'ring-1 ring-inset ring-white/30' : ''}`}>
                    <span className={`text-xs font-mono w-6 h-6 flex items-center justify-center rounded-full ${isToday ? 'bg-white text-black font-bold' : 'text-gray-400'}`}>{day}</span>
                    <div className="flex flex-wrap gap-0.5 mt-0.5 justify-center">
                      {dayMeetings.slice(0,3).map(m => <span key={m.id} className={`h-1 w-1 rounded-full ${TYPE_DOT[m.type]}`} />)}
                    </div>
                  </button>
                )
              })}
                  ...
                  <div key={m.id} className={`flex items-center gap-3 rounded-lg border px-3 py-2.5 ${TYPE_COLORS[m.type]}`}>
                    ...
                    {m.link && (
                      <a href={m.link} target="_blank" rel="noreferrer"
                        className="rounded border border-current px-2 py-1 text-[10px] font-semibold hover:opacity-80 flex items-center gap-1">
                        <Video className="h-3 w-3" /> Join
                      </a>
                    )}
                    <button onClick={() => handleDelete(m.id)} className="opacity-60 hover:opacity-100 transition">
          ...
          <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {upcomingMeetings.map(m => (
                    <div key={m.id} className="flex items-center gap-2.5 group">
                      <span className={`h-2 w-2 rounded-full shrink-0 ${TYPE_DOT[m.type]}`} />
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                        {m.link && (
                          <a href={m.link} target="_blank" rel="noreferrer"
                            className="rounded border border-white/10 p-1 text-gray-500 hover:text-white">
                            <Video className="h-3 w-3" />
                          </a>
                        )}
                        <button onClick={() => handleDelete(m.id)} className="rounded border border-red-500/20 p-1 text-red-400/60 hover:text-red-400">
            ...
            <div className="rounded-2xl border border-white/10 bg-[#0d0d14] p-5">
              <p className="text-xs font-mono uppercase tracking-widest text-gray-600 mb-3">Types</p>
              <div className="space-y-2">
                {[['review','Code Review'],['planning','Planning'],['retro','Retrospective'],['sync','Team Sync']].map(([type, label]) => (
