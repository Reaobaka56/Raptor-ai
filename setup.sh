...
if command -v pg_isready &> /dev/null; then
    if pg_isready -q; then
        ...
        echo "⚠️  PostgreSQL is not running. Start it and run: npx prisma migrate dev --name init"
    fi
else
    echo "⚠️  PostgreSQL not found. Install PostgreSQL and run: npx prisma migrate dev --name init"
fi
